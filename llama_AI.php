...
$chunks = LlamaDocumentSplitter::split($content, 1000, 200, 'sentences');

// Process with metadata
foreach ($docs as $doc) {
    $doc->metadata['processed_at'] = date('c');
    $doc->metadata['source_system'] = 'document_pipeline';
}
```
...
} catch (LlamaRateLimitException $e) {
    // Handle rate limiting
    sleep(60);
    $response = $chat->ask($question);
} catch (LlamaTimeoutException $e) {
    // Handle timeouts
    $response = "Sorry, the request timed out. Please try again.";
} catch (LlamaSecurityException $e) {
...
class LlamaExperiment {
    public function runABTest(string $question): array {
        $providers = ['openai', 'anthropic'];
        $results = [];
        
        foreach ($providers as $provider) {
            $chat = new LlamaChat($provider);
            ...
            $results[$provider] = [
                'response' => $response,
                'duration' => $duration,
                'tokens' => $chat->getUsage()['total_tokens'],
            ];
        }
...
### 12. Custom Vector Store Implementation
```php
class LlamaRedisVectorStore extends LlamaBaseVectorStore {
    private $redis;
    ...
    public function add(string $id, array $vector, string $content, array $metadata = []): void {
        ...
        $this->redis->hSet('vectors', $id, json_encode($data));
    }
    
    public function search(array $vector, int $limit = 5, array $filters = []): array {
        // Implementation using Redis search capabilities
        // This would require Redis modules like RediSearch
        return [];
    }
...
class LlamaConnectionPool {
    ...
    public static function getConnection(string $provider): LlamaChatInterface {
        if (!isset(self::$pools[$provider])) {
            self::$pools[$provider] = match($provider) {
...
### 2. Async Processing
```php
class LlamaAsyncProcessor {
    public function processDocumentsAsync(array $documents): void {
        ...
        // Wait for all to complete (pseudo-code)
        Promise::all($promises)->wait();
    }
    
    private function processChunkAsync(array $chunk): Promise {
        // Implementation would depend on your async library
        return new Promise(function($resolve) use ($chunk) {
...
### 3. Caching Strategy
```php
class LlamaSmartCache {
    private LlamaCacheInterface $l1Cache; // Memory
    private LlamaCacheInterface $l2Cache; // Redis
    ...
    public function get(string $key): mixed {
        ...
        if ($value !== null) {
            $this->l1Cache->set($key, $value, 300); // Cache in L1
            return $value;
        }
...
LlamaConfig::setSecret('openai_api_key', $_ENV['OPENAI_API_KEY']);

// Rotate keys regularly
class LlamaKeyRotator {
    public function rotateKeys(): void {
        $newKey = $this->fetchNewKeyFromVault();
        LlamaConfig::setSecret('openai_api_key', $newKey);
...
```php
class LlamaSecureInput {
    public static function sanitize(string $input): string {
        // Remove potentially dangerous content
        $input = preg_replace('/[^\p{L}\p{N}\s\p{P}]/u', '', $input);
        
        // Limit length
        $maxLength = LlamaConfig::get('security.max_input_length');
        if (strlen($input) > $maxLength) {
            throw new LlamaSecurityException("Input too long");
        }
...
```php
class LlamaUserRateLimiter extends LlamaRateLimiter {
    public function checkUserLimit(string $userId, int $requestCost = 1): void {
        $userTier = $this->getUserTier($userId);
        $limits = $this->getTierLimits($userTier);
        
        parent::checkRequest($userId);
...
### 1. Custom Metrics
```php
class LlamaCustomMetrics extends LlamaMetrics {
    public static function recordBusinessMetric(string $name, float $value, array $tags = []): void {
        parent::gauge("business.$name", $value, $tags);
        
        // Send to external monitoring
        if (self::shouldSendToExternal()) {
            self::sendToDatadog($name, $value, $tags);
        }
    }
}
```
...
class LlamaTracing {
    ...
    public static function startTrace(string $operation): string {
        self::$traceId = uniqid('trace_');
        self::addSpan($operation, 'start');
        return self::$traceId;
    }
    
    public static function addSpan(string $operation, string $event): void {
        $span = [
...
### 1. Unit Tests
```php
class LlamaChatTest extends PHPUnit\Framework\TestCase {
    public function testBasicChat(): void {
        $mockClient = $this->createMock(LlamaHttpClient::class);
        $mockClient->method('post')->willReturn(['choices' => [['message' => ['content' => 'Test response']]]]);
        
        $chat = new LlamaOpenAIChat();
        $chat->setHttpClient($mockClient);
...
### 2. Integration Tests  
```php
class LlamaIntegrationTest extends PHPUnit\Framework\TestCase {
    public function testFullRAGPipeline(): void {
        $rag = new LlamaRAG('openai', 'openai', 'memory');
        $rag->addDocument('PHP is a programming language');
        
        $response = $rag->ask('What is PHP?');
        $this->assertStringContainsString('programming', $response);
    }
}
```
...
This enterprise-grade Llama.php framework provides:

✅ **Multi-Provider Support**: OpenAI, Anthropic, Ollama, Google Gemini
✅ **Advanced RAG**: Multiple vector stores, reranking, multi-query
✅ **Enterprise Security**: Input validation, prompt injection detection, rate limiting
✅ **Production Ready**: Logging, metrics, health checks, error handling
✅ **Scalable Architecture**: Connection pooling, caching, async processing
✅ **Monitoring**: Comprehensive logging, metrics, distributed tracing
✅ **Testing**: Unit and integration test support
✅ **Documentation**: Complete examples and deployment guides

The framework is designed to handle production workloads with proper error handling, security measures, and observability features that enterprise applications require.
*/ PHP 8.1+ required
